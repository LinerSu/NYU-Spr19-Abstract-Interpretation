...
let print_labelling (at, atP, after, aftP, escape, break, brkP) m =
    let s = "<" ^ (string_of_label at) ^ ": " ^ (stringofaP atP) ^ "; " ^ (string_of_label after) ^ ": " ^ (stringofaP aftP) ^ "; " ^ (bool_to_string escape) ^ "; " ^ (string_of_label break) ^ ": " ^ (stringofaP brkP) ^ ">"  in
       print_string s;
       if String.length s < labelmargin then print_space (labelmargin - (String.length s)+ 1);
       print_space m;;
let print_at (at, atP, after, aftP, escape, break, brkP) = print_label at;;
let print_after (at, atP, after, aftP, escape, break, brkP) = print_label after;;
